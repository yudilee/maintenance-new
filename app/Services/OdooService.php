...
class OdooService
{
    ...
    public function fetchRepairHistory(string $lotNumber): array
    {
        try {
            ...
            // 2. Search recent repair orders for this lot (starting from Nov 30, 2025)
            $repairIds = $this->execute('repair.order', 'search', [
                [['lot_id', '=', $lotId], ['create_date', '>=', '2025-11-30 00:00:00']]
            ]);
     ...
     */
    public function enrichWithRepairData(): void
    {
        $serviceLocations = [
        ...
        $lotMap = $this->resolveLotIds($inServiceItems);
        ...
        $repairData = $this->fetchRepairOrders($lotMap);
